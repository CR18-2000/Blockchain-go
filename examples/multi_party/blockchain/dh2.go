...
func DH2Handler(quitChan chan struct{}, wg *sync.WaitGroup, contracts map[string]myContract) {
	logger := log.New(os.Stdout, "[DH2 daemon] : ", 0)
	defer wg.Done()
	...
	contractAddress := common.HexToAddress(contracts["DH2"].address.Hex())
	contractABI, err := abi.JSON(strings.NewReader(contracts["DH2"].abi))
	if err != nil {
