...
func DH1Handler(quitChan chan struct{}, wg *sync.WaitGroup, contracts map[string]myContract) {
	...
	contractAddress := common.HexToAddress(contracts["DH1"].address.Hex())
	contractABI, err := abi.JSON(strings.NewReader(contracts["DH1"].abi))
	if err != nil {
	...
	for {
		...
		case vLog := <-eventCh:
			...
			case "receiveKeysEvent":
				...
				//callSCMethod(contracts["DH1"], "[DH1] : ", "receiveKeys", paramsBytes, pkBytes, PZeroBytes, POneBytes, addresses)
			}
		case <-quitChan:
			// Exit the function
